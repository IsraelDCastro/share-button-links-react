...
interface ButtonLinkedInIconProps extends BooleanButtonIconOpts {
  url: string;
}
...
export default function ButtonLinkedInIcon({
  ...
  validateUrl = false,
  fallbackUrl
}: ButtonLinkedInIconProps) {
  const shareUrl = resolveShareUrl(url, { validateUrl, fallbackUrl });
  const href = buildShareUrl("https://www.linkedin.com/sharing/share-offsite/", { url: shareUrl });

  return (
    <a
      href={href}
      className={getIconButtonClassNames("btn-link-icon btn-link-linkedin-icon", {
        isRounded,
        isAllWhite,
        isWhited,
        isBordered,
