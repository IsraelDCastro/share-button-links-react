...
export default function ButtonPocketIcon ({ url, title, isRounded }) {
	return (
		<a href={ `https://getpocket.com/save?url=${url}&title=${title}` }
		   className={ `btn-link-icon btn-link-pocket-icon ${isRounded ? "is-rounded" : null}` }
		   title="Pocket"
		   rel="noreferrer nofollow noopener" target="_blank">
			<svg aria-hidden="true" focusable="false" data-prefix="fab" data-icon="get-pocket"
			     className="svg-inline--fa fa-get-pocket fa-w-14" role="img" xmlns="http://www.w3.org/2000/svg"
			     viewBox="0 0 448 512">
